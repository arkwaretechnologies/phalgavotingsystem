...
export function VotedVotersClient({
  rows,
  generatedAtIso,
}: {
  rows: VotedVoterRow[];
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      const hay = [r.full_name, r.position, r.lgu, r.province, r.email, r.phone]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
  ...
  function exportCsv() {
    ...
    a.href = url;
    a.download = `voted-voters_${new Date(generatedAtIso).toISOString().slice(0, 10)}.csv`;
  ...
  function exportPdfPrint() {
    const a = document.createElement("a");
    a.href = "/admin/reports/voted-voters/pdf";
    ...
    <div className="space-y-6">
      <div className="rounded-2xl border border-neutral-200/80 bg-white p-6 shadow-sm">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-xl font-semibold">Voted Voters</h1>
            <p className="mt-2 text-sm text-neutral-600">
              Voters with a submitted ballot (casted their vote). Generated{" "}
